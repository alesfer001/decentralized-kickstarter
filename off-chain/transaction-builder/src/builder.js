...
const ckbClient_1 = require("./ckbClient");
/**
 * Transaction builder for creating campaigns and pledges
 */
class TransactionBuilder {
    constructor(client, campaignContract, campaignLockContract, pledgeContract, pledgeLockContract, receiptContract) {
        this.client = client;
     ...
     */
    async createCampaign(signer, params) {
        console.log("Building create campaign transaction...");
        // Serialize campaign data
        const campaignData = (0, serializer_1.serializeCampaignData)(params);
        console.log(`Campaign data: ${campaignData}`);
        // Calculate required capacity (65 bytes header + metadata)
        const metadataSize = (params.title || params.description) ? (0, serializer_1.getMetadataSize)(params.title, params.description) : 0;
        const dataSize = 65 + metadataSize;
        const capacity = (0, serializer_1.calculateCellCapacity)(dataSize, true, 65);
        ...
        const deadlineArgs = (0, serializer_1.encodeDeadlineBlockAsLockArgs)(params.deadlineBlock);
        ...
        const firstInput = core_1.ccc.CellInput.from(tx.inputs[0]);
        const serializedInput = firstInput.toBytes();
        const outputIndexBytes = core_1.ccc.numLeToBytes(0, 8); // campaign is output[0]
        const hasher = new core_1.ccc.HasherCkb();
     ...
     * @returns Transaction hash
     */
    async createPledge(signer, params) {
        console.log("Building create pledge transaction...");
        // Serialize pledge data
        const pledgeData = (0, serializer_1.serializePledgeData)(params);
        ...
        const baseCapacity = (0, serializer_1.calculateCellCapacity)(dataSize, true, 65);
        ...
        const lockScript = (await core_1.ccc.Address.fromString(lock, this.client)).script;
        // Build the transaction
        const tx = core_1.ccc.Transaction.from({
            outputs: [
                {
     ...
     */
    async finalizeCampaign(signer, params) {
        console.log("Building finalize campaign transaction...");
        // Serialize new campaign data with updated status
        const newCampaignData = (0, serializer_1.serializeCampaignDataWithStatus)(params.campaignData, params.newStatus);
        console.log(`New campaign data: ${newCampaignData}`);
        // Calculate minimum capacity for the campaign cell (65 bytes header + metadata)
        const metadataSize = (params.campaignData.title || params.campaignData.description) ? (0, serializer_1.getMetadataSize)(params.campaignData.title, params.campaignData.description) : 0;
        const dataSize = 65 + metadataSize;
        const minCapacity = (0, serializer_1.calculateCellCapacity)(dataSize, true, 65);
        ...
        const deadlineArgs = (0, serializer_1.encodeDeadlineBlockAsLockArgs)(deadlineBlock);
        ...
        const outputsData = [newCampaignData];
     ...
     */
    async refundPledge(signer, params) {
        console.log("Building refund pledge transaction...");
        // Get the backer's lock script
        const address = await signer.getRecommendedAddress();
        const lockScript = (await core_1.ccc.Address.fromString(address, this.client)).script;
     ...
     */
    async releasePledgeToCreator(signer, params) {
        console.log("Building release pledge transaction...");
        // Get the creator's lock script from their address
        const creatorLockScript = (await core_1.ccc.Address.fromString(params.creatorAddress, this.client)).script;
        // Get the backer's lock script (for change)
        const backerAddress = await signer.getRecommendedAddress();
        const backerLockScript = (await core_1.ccc.Address.fromString(backerAddress, this.client)).script;
     ...
     */
    async destroyCampaign(signer, params) {
     ...
     */
    async createPledgeWithReceipt(signer, params) {
        ...
        const pledgeData = (0, serializer_1.serializePledgeData)({
            campaignId: params.campaignId,
            backerLockHash: params.backerLockHash,
            amount: params.amount,
        });
        // Serialize receipt cell data (40 bytes): pledge_amount + backer_lock_hash
        const receiptData = (0, serializer_1.serializeReceiptData)(params.amount, params.backerLockHash);
        // Serialize pledge lock args (72 bytes): campaign_type_script_hash + deadline + backer_lock_hash
        const pledgeLockArgs = (0, serializer_1.serializePledgeLockArgs)(params.campaignTypeScriptHash, params.deadlineBlock, params.backerLockHash);
        // Calculate capacities
        const pledgeDataSize = 72;
        const pledgeBaseCapacity = (0, serializer_1.calculateCellCapacity)(pledgeDataSize, true, 65);
        const pledgeTotalCapacity = pledgeBaseCapacity + params.amount;
        const receiptDataSize = 40;
        const receiptCapacity = (0, serializer_1.calculateCellCapacity)(receiptDataSize, true, 65);
        ...
        const pledgeTypeScriptArgs = core_1.ccc.hexFrom(this.receiptContract.codeHash.slice(2)); // Remove 0x prefix
        const receiptTypeScriptArgs = core_1.ccc.hexFrom(this.pledgeContract.codeHash.slice(2)); // Remove 0x prefix
        const tx = core_1.ccc.Transaction.from({
            outputs: [
                {
     ...
     */
    async permissionlessRelease(signer, params) {
            ...
            outputs: [
                {
                    ...
                    lock: {
                        ...
                        hashType: params.creatorLockScript.hashType,
     ...
     */
    async permissionlessRefund(signer, params) {
            ...
            outputs: [
                {
                    ...
                    lock: {
                        ...
                        hashType: params.backerLockScript.hashType,
     ...
     */
    async mergeContributions(signer, params) {
        ...
        const mergedPledgeData = (0, serializer_1.serializePledgeData)({
            campaignId: params.campaignId,
            backerLockHash: params.backerLockHash,
     ...
     */
    getCampaignLockContract() {
        return this.campaignLockContract;
    }
    /**
     * Helper: Get lock hash from address
     */
    async getLockHashFromAddress(address) {
        const addr = await core_1.ccc.Address.fromString(address, this.client);
 ...
 */
function createTransactionBuilder(rpcUrl, campaignContract, campaignLockContract, pledgeContract, pledgeLockContract, receiptContract, network) {
    // Auto-detect network if not explicitly specified
    const resolvedNetwork = network ?? ((rpcUrl.includes("127.0.0.1") || rpcUrl.includes("localhost")) ? "devnet" : "testnet");
    const client = (0, ckbClient_1.createCkbClient)(resolvedNetwork, rpcUrl);
