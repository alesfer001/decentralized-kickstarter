...
- **Not done:** `test-lifecycle.ts` / `test-v1.1-*.ts` still not re-run. On page load the first click or keystroke is sometimes ignored, consistent with the pre-existing `<Next.Metadata>` hydration error regenerating the tree.
