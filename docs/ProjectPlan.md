...
- **Verified on devnet:** exploits 4/4 rejected, accounting 36/36 (backer's whole-lifecycle cost for a successful 150 CKB pledge is 150 CKB + 0.002 CKB fees; was 616 CKB), accumulator 18/18, browser pledge accepted with the 80-byte receipt, bot release of a 100 CKB pledge paid creator 100 / backer 251.999.
- **Open — pledges under ~61 CKB cannot be released.** A cell needs 61+ CKB to exist, so a creator output of exactly `amount` fails for small pledges (bot release of a 20 CKB pledge: `InsufficientCellCapacity`). Needs a decision before testnet redeploy.
